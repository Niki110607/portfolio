...
export default function InfinitePage() {
  ...
  return (
    ...
    >
      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-[850px] w-[850px] rounded-full bg-[var(--color-accent-glow)] opacity-20 blur-[220px] pointer-events-none" />
      ...
      <main className="relative z-10 flex-1 w-full max-w-5xl mx-auto px-6 pb-16">
        ...
        <div className="flex items-center justify-between px-1 pb-3 border-b border-zinc-800/80">
          ...
          <div className="flex items-center gap-2">
            <span className="h-2 w-2 rounded-full bg-[var(--color-accent)] animate-pulse shadow-[0_0_10px_var(--color-accent-glow)]" />
        ...
        <section className="w-full mt-10 py-5 border-y border-zinc-800/70">
        ...
        <div className="flex justify-center mt-10">
          <button
            ...
            className="flex items-center gap-2 rounded-full border border-zinc-800 bg-zinc-900/30 px-4 py-2 text-xs font-mono text-zinc-500 transition-all hover:border-zinc-700 hover:text-zinc-200"
          >
            <span>
              {showTechDetails
                ? langIsGerman
      ...
      {showTechDetails && (
        <footer className="relative z-10 w-full border-t border-zinc-800/80 bg-zinc-950/95 backdrop-blur-xl py-10">
          <div className="max-w-5xl mx-auto px-6">
            <div className="flex gap-6 overflow-x-auto border-b border-zinc-800/80 text-xs font-mono mb-7">
              <button
                onClick={() => setActiveTab("overview")}
                className={`${activeTab === "overview" ? "border-[var(--color-accent)] text-[var(--color-accent)] font-bold" : "border-transparent text-zinc-500 hover:text-zinc-300"} whitespace-nowrap border-b-2 pb-3 transition-colors`}
              >
                {langIsGerman ? "Kombinationsablauf" : "Combination Flow"}
              </button>
              <button
                onClick={() => setActiveTab("llm")}
                className={`${activeTab === "llm" ? "border-[var(--color-accent)] text-[var(--color-accent)] font-bold" : "border-transparent text-zinc-500 hover:text-zinc-300"} whitespace-nowrap border-b-2 pb-3 transition-colors`}
              >
